...
# (ordered) list of files to be combined and delivered to the client
CLIENT_JS_SRCS ?= lib/ageism-util.js lib/status-code.js lib/mock-datasource.js lib/game-engine.js
# destination for combined client-side js files
CLIENT_JS_FILE ?= client-lib/ageism-client-complete.js
# destination for minimized version of client side js files
MINIMIZED_CLIENT_JS_FILE ?= client-lib/ageism-client-complete.min.js
# executable by which to minimize the js fiels
UGLIFY_EXE ?= uglifyjs
# args to pass to uglify
UGLIFY_ARGS ?= 
...
clean: clean-coverage clean-docco clean-docs clean-js clean-module clean-node-modules clean-client-js
...
client-js: $(CLIENT_JS_SRCS)
	mkdir -p `dirname $(CLIENT_JS_FILE)`
	echo "/* ageism files compiled on `date` */" > $(CLIENT_JS_FILE)
	echo "" >> $(CLIENT_JS_FILE)
	for jsfile in $(CLIENT_JS_SRCS) ; do \
		echo "\n/* file:$$jsfile */\n" >> $(CLIENT_JS_FILE); \
		cat $$jsfile >> $(CLIENT_JS_FILE); \
	done
	echo "\n/* EOF */\n" >> $(CLIENT_JS_FILE)

minimize-client-js: client-js
	$(UGLIFY_EXE) $(UGLIFY_ARGS) --output $(MINIMIZED_CLIENT_JS_FILE) $(CLIENT_JS_FILE)

clean-client-js:
	rm -r $(RM_DASH_I) $(MINIMIZED_CLIENT_JS_FILE) $(CLIENT_JS_FILE)
